...
# HDFS paths
INPUT=/camelyon/input/manifest_rawCML.csv
OUT_BG=/camelyon/out_bg
OUT_SORT=/camelyon/out_sort
OUT_DEDUP=/camelyon/out_dedup

LOCAL_OUT=mr_camelyon
mkdir -p $LOCAL_OUT
...
# ===========================
chmod +x mr_camelyon/scripts/*.py
...
hadoop jar "$HADOOP_STREAMING_JAR" \
  -files mr_camelyon/scripts/mapper_bgfilter.py,mr_camelyon/scripts/reducer_passthrough.py \
...
START_JOB2=$(date +%s)
hadoop jar "$HADOOP_STREAMING_JAR" \
  -files mr_camelyon/scripts/mapper_group.py,mr_camelyon/scripts/reducer_group_sort.py \
...
START_JOB3=$(date +%s)
hadoop jar "$HADOOP_STREAMING_JAR" \
  -files mr_camelyon/scripts/mapper_window.py,mr_camelyon/scripts/reducer_window.py \
...
# Stats
# ===========================
TOTAL=$(($(wc -l < data/CAMELYON/manifest_rawCML.csv) - 1))
...
cat > $LOCAL_OUT/mr_camelyon_stats.json <<EOF
{
  "total_patches": $TOTAL,
...
echo "  - $LOCAL_OUT/mr_camelyon_stats.json"